...
void decrypt(char* decryptMessage, char* message, char* key)
{
    ...
    for (int i = 0; i < size; i++) {
        messageNum[i] = (charToNumber(message[i]) - charToNumber(key[i]));
        // Checking if messageNum[] is negative and account for it
        if (messageNum[i] < 0) {
            messageNum[i] = messageNum[i] + 27;
        }
        messageNum[i] = messageNum[i] % 27;
    }
    ...
    {
        decryptMessage[j] = numberToChar(messageNum[j]);
    }
}
    ...
    while (1) {
        ...
        case 0: {
            ...
            if (strcmp(clientID, "dec") != 0) {
                ...
                while (1) {
                    charsRead = recv(connectionSocket, p, 141000, 0);
                ...
                const char* pm = decryptMessage;
                int length = strlen(decryptMessage);
                charsRead = 0;
                while (length > 0) {
